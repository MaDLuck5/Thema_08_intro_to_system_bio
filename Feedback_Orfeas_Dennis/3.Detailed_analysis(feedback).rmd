...
> Please make sure to declare variables with the same method, D01 and D03 should also be assigned with an <- instead of an =.
...
>How do you know that steady state one always hits on T=30 and it seems like the implimentation of D = 0 doesn't fully work with a ifelse statement inside the model fucntion, have u looked at the event and root parameters u can give to the ODE function? Also your model doesn't stop when the second steady state with D = 0 is hit.
...
> Try plotting all the lines in one graph, they are very hard to read and compare right now
...
> try plotting all the lines in one graph, they are very hard to read and compare right now, (again)
> Also, double check when using = or proper declaration of variables R.
