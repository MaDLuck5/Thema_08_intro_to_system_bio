...
dataset_low_dose <- subset(medians, dose == 0.1)
dataset_high_dose <- subset(medians, dose == 0.3)
head(dataset_low_dose)
head(dataset_high_dose)
```
```{r}
library("deSolve")
...
plot(as.data.frame(out)$Rmo, type = "line", xlab = "Time in Hours", ylab = "Rmo concentration", ylim =  c(0.5,5.0))
lines(mRNA~time, data = dataset_low_dose, col = "red")


plot(as.data.frame(out)$Ro, type = "line", xlab = "Time in Hours", ylab = "Ro concentration")
lines(Free_receptor~time, data = dataset_low_dose, col = "red")

plot(as.data.frame(out)$DR, type = "line", xlab = "Time in Hours", ylab = "DR concentration")


plot(as.data.frame(out)$DRN, type = "line", xlab = "Time in Hours", ylab = "DRN concentration")
