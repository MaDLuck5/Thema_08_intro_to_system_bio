...
plot(as.data.frame(out)$Ib, type = "l", xlab = "Time in Days", ylab = "Bubonice indices", col = "blue", ylim = c(0, 30), main = "Deterministic model")
plot(as.data.frame(out)$Ip, type = "l", xlab = "Time in Days", ylab = "Puneonic indices", col = "blue", ylim = c(0, 120), main = "Deterministic model")
...
# to test sample diffreance with a simple T test
...
```
```{r,include=TRUE, fig.align="center", fig.cap=c("Python code figure"), echo=FALSE}
...
Looking at the deterministic models from both Python and R, it shows that the graphs are almost similar. Although, due to there being differences in how both coding-styles work under the hood. The graphs are different as is shown in the comparison between the deterministic models of R & python.
