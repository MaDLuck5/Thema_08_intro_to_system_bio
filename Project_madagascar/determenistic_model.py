...
__author__ = "Mats Slik & Dennis Haandrikman"
...
def deterministic_model(x, tijd):
    ...
    :param x: list of  starting state parameters
    :param tijd: sequence of each time point
    :return: the delta values of each state parameter
    """
    flea_t = flea(tijd)
...
# exporting a file to read in R

with open('deterministic_model_data.csv', 'w',
          encoding='UTF8') as f:
    writer = csv.writer(f)
...
plt.tight_layout()
plt.savefig('Deterministic_Fig1.pdf', dpi=300,
            bbox_inches='tight')
